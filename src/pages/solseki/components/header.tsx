...
export function SolsekiMainInfo() {
  return (
    <HelpPanel
      header={<h2>Solseki</h2>}
      footer={
        <ExternalLinkGroup
          items={[
...
export function SolsekiHeader({ actions }: { actions: React.ReactNode }) {
  const loadHelpPanelContent = useHelpPanel();
  return (
    <Header
      variant="h1"
      info={<InfoLink onFollow={() => loadHelpPanelContent(<SolsekiMainInfo />)} />}
      actions={actions}
    >
      Dashboard
    </Header>
