...
export function MusubiAuthMainInfo() {
  return (
    <HelpPanel
      header={<h2>Musubi Auth</h2>}
      footer={
        <ExternalLinkGroup
          items={[
...
export function MusubiAuthHeader({ actions }: { actions: React.ReactNode }) {
  const loadHelpPanelContent = useHelpPanel();
  return (
    <Header
      variant="h1"
      info={<InfoLink onFollow={() => loadHelpPanelContent(<MusubiAuthMainInfo />)} />}
      actions={actions}
    >
      Dashboard
    </Header>
